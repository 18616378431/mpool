...
namespace
{
	static uint32 SizeForType(MYSQL_FIELD* field)
	{
		...
		default:
			std::cout <<
				mpool::StringFormatFmt("SQL::SizeForType(): invalid field type {}", uint32(field->type)) << std::endl;
			return 0;
	...
	DatabaseFieldTypes MysqlTypeToFieldType(enum_field_types type)
	{
		...
		default:
			std::cout <<
				mpool::StringFormatFmt("MysqlTypeToFieldType(): invalid field type {}", uint32(type)) << std::endl;
			break;
		}
...
bool ResultSet::NextRow()
{
	...
	{
		std::cout <<
			mpool::StringFormatFmt("{}, mysql_fetch_lengths, cannot retrieve value lengths.Error {}.",
				__FUNCTION__, mysql_error(_result->handle)) << std::endl;
		CleanUp();
		return false;
	}
	...
	m_metadataResult(result)
{
	...
	{
		std::cout <<
			mpool::StringFormatFmt("{}:mysql_stmt_store_result, cannot bind result from MySQL server.Error {}.",
				__FUNCTION__, mysql_stmt_error(m_stmt)) << std::endl;
		delete[] m_rBind;
		delete[] m_isNull;
		delete[] m_length;
		return;
	...
	{
		std::cout <<
			mpool::StringFormatFmt("{}:mysql_stmt_bind_result, cannot bind result from MySQL server.Error {}.",
				__FUNCTION__, mysql_stmt_error(m_stmt)) << std::endl;
