...
{
	// this value stores somewhat structured data that need function style casting
	data.value = newValue;
	data.length = length;
	data.raw = false;
}
...
void Field::LogWrongType(std::string_view getter, std::string_view typeName) const
{
	...
	std::cout <<
		mpool::StringFormatFmt("Warning: {}<{}> on {} field {}.{} ({}.{}) at index {}.",
			getter, typeName, meta->TypeName, meta->TableAlias, meta->Alias, meta->TableName, meta->Name, meta->Index) << std::endl;
}
...
{
	static_assert(std::is_arithmetic_v<T>, "Unsupport type for Field::GetData()");

	if (!data.value)
		return GetDefaultValue<T>();

#ifdef POOL_STRICT_DATABASE_TYPE_CHECKS
	...
	if (data.raw)
		...
		result = mpool::StringTo<T>(data.value);
	...
	if constexpr (std::is_same_v<T, double>)
	{
		if (data.raw && !IsType(DatabaseFieldTypes::Decimal))
			...
			result = mpool::StringTo<float>(data.value);
	}
	...
	if constexpr (std::is_same_v<T, uint32>)
	{
		...
		{
			auto signedResult = mpool::StringTo<int32>(data.value);
