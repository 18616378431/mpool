...
//conn
CMyTcpConnection::CMyTcpConnection(boost::asio::io_context & ser) :m_nSocket(ser)
{

}

CMyTcpConnection::~CMyTcpConnection()
{
    cout << "~CMyTcpConnection" << endl;
}

CMyTcpConnection::CPMyTcpCon CMyTcpConnection::CreateNew(boost::asio::io_context& io_service)
{
    return CPMyTcpCon(new CMyTcpConnection(io_service));
}

void CMyTcpConnection::AsyncRead()
{
...
void CMyTcpConnection::handle_read(boost::system::error_code error, size_t transferredBytes)
{
...
std::string CMyTcpConnection::GetRemoteIpAddress() const
{
    return m_nSocket.remote_endpoint().address().to_string();
}

void CMyTcpConnection::printHex(const unsigned char* data, size_t size)
{
...
tcp::socket& CMyTcpConnection::socket()
{
    return m_nSocket;
}

void CMyTcpConnection::handle_write()
{
...
void CMyTcpConnection::HandleWrite(const boost::system::error_code & error, std::size_t bytes_transferred)
{
    if (!error)
    {
...
bool CMyTcpConnection::VerifyVersion(uint8 const* a, int32 aLength, mpool::Crypto::SHA1::Digest const& versionProof, bool isReconnect)
{
...
//server
CMyService::CMyService(const string& strIP, const int& port, int nThreads)
...
boost::asio::io_context& CMyService::get_io_service()
{
    return GET_IO_SERVICE(m_tcpAcceptor);
}

void CMyService::HandleAccept(const boost::system::error_code& error,
