...
namespace mpool::Impl::StringConvertImpl
{
	template <typename T, typename = void> struct For
	{
		static_assert(mpool::dependant_false_v<T>, "Unsupported type used for ToString or StringTo");
	};
...
#if POOL_PLATFORM == POOL_COMPILER_MICROSOFT
	template<typename T>
...
namespace mpool
{
	...
	{
		return mpool::Impl::StringConvertImpl::For<Result>::FromString(str, std::forward<Params>(params)...);
	}

	template<typename Type, typename... Params>
	std::string ToString(Type&& val, Params&&... params)
	{
		return mpool::Impl::StringConvertImpl::For<std::decay_t<Type>>::ToString(std::forward<Type>(val), std::forward<Params>(params)...);
	}
}
